...
public class PongExtension extends Canvas implements KeyListener, Runnable
{
	...
	public PongExtension()
	{
		...
		ball = new Ball(350, 200, 10, 10, Color.BLACK, 4, 4);
		mainPaddle = new PaddleExtension(375, 275, 40, 40, Color.BLACK, 2);
		//blocks on the left
		...
		for (int x = 85; x < 675; x += 80) {
			for (int y = 5; y < 85; y += 40) {
				blocks.add(new Block(x, y, 75, 35, Color.CYAN));
			}
		}
   ...
   public void paint(Graphics window)
   {
		...
		if (blocks.size() == 0) {
			if (!played) {
				//set up all variables related to the game
				paddleX = mainPaddle.getX();
				paddleY = mainPaddle.getY();
				ball = new Ball(350, 200, 10, 10, Color.BLACK, 4, 4);
				mainPaddle = new PaddleExtension(paddleX, paddleY, 40, 40, Color.BLACK, 2);
				//blocks on the left
				for (int x = 5; x < 85; x += 40) {
					for (int y = 5; y < 600-55; y += 80) {
						blocks.add(new Block(x, y, 35, 75, Color.RED));
